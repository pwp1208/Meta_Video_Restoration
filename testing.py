...
def generate_images(model, test_input, tar,filename,mode='test'):
    ...
    for j in range(1,test_input.shape[0]):
        ...
        try:
            os.mkdir(config.output_path + filename +'/')
        except:
            pass
        ...
        new_im.save(config.output_path + filename +'/'+str(j)+'.jpg')
...
for i,filename in enumerate(test_task):
    ...
    generate_images(generator, gen_input, target, filename[:-3], mode='test')
    print('n=',n,end='/r')
    n+=1
